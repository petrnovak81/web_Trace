<div id="SVZmenaD1MAX" 
    data-role="window"
    data-title="@Html.Raw(Language.ZmenaD1MAX_Title1)"
    data-width="600"
    data-modal="true"
    data-resizable="false"
    data-actions="['close']"
    style="display: none;">
    <h3>@Html.Raw(Language.ZmenaD1MAX_Title2)</h3>
    <div data-role="grid" 
        data-resizable="true"
    ...
    <h4>@Html.Raw(Language.SVZmenaD1MAXTXT1)</h4>
    <input style="width: 100%;" data-role="datepicker" data-bind="value: D1MAX, mindate: today" placeholder="@Html.Raw(Language.SVZmenaD1MAXTXT1)" />
    <textarea class="k-textbox" maxlength="300" autofocus style="width:100%;max-width:100%;" rows="3" data-bind="value: Comment" placeholder="@Html.Raw(Language.SVZmenaD1MAXPLH3)"></textarea>
    <div class="window-footer">
        <button type="button" class="k-primary k-button" data-bind="events: { click: Save }">@Language.btnD1MAX</button>
        <button type="button" class="k-button" data-bind="events: { click: Storno }">@Language.btnStorno</button>
    </div>
</div>

<script>
    function SVZmenaD1MAX(items, callback) {
        var iDs = {};
        var win;
        ...
        var model = new kendo.observable({
            ...
            Save: function (e) {
                if (!$.trim(this.Comment)) {
                    message("   @SystemMessages.NoComment5 ", "info")
                    return;
                };
                ...
                var promises = res.map(function (id, i) {
                    return $.Deferred(function (d) {
                        $.get("@Url.Action("sp_Do_Super2_3_4", "Api/AdminService")", {
                            iDSpisu: id,
                            d1MAX: kendo.toString(that.D1MAX, "yyyy-MM-dd HH:mm:ss"),
        ...
        kendo.bind($("#SVZmenaD1MAX"), model);
        win = $("#SVZmenaD1MAX").data("kendoWindow");
        win.open().center();
    };
</script>
