    ...
    <div id="splitter-right-pane" class="flex-body" style="overflow: hidden;">
        ...
        <div class="actionbuttons" id="actionbuttons" style="height: 100%; width: 100%; overflow-x: hidden" class="k-scrollable">
                ...
                Select Case Html.CurAction
                    ...
                    Case "PersonalVisit"
                        ...
                        'zapis OSN
                                            ...
                                            @<a data-role="button" href="@Url.Action("AllCasesFromTRACE", "Home")?act=false">Stáhnout aktivní spisy</a>
            ...
            Case "Finished"
                                                                                                        ...
                                                                                                        @<a data-role="button" href="@Url.Action("AllCasesFromTRACE", "Home")?act=true">Stáhnout ukončené spisy</a>
                End Select
            End Code
        </div>
        <!-- end buttons -->
...
<script>
   ...
   //datovy model
    var viewModel = kendo.observable({
                    ...
                    case 37: //Ukoncene zados o vraceni spisu
                        ...
                        showReqFileReturn(function (c) {
                            $.get('@Url.Action("FinishedPostEmail", "Api/Service")', { iDSpisu: items[0].IDSpisu, comment: c }, function (result) {
                                if (result.MsgType === "error") {
                                    console.log("FinishedPostEmail Error");
        ...
        traceplaneradio: function (e) {
            ...
            globalValidAddress(addr, function (adr, bol, msg) {
                ...
                var GPSValid = false;
                if (!bol) {
                    console.log("globalValidAddress Error");
                    message(msg, "error");
                } else {
                ...
                $.get("@Url.Action("sp_Do_ChangeAddrForItinerary", "Api/Service")", {
                    ...
                    if (!result.OK) {
                        ...
                        console.log("sp_Do_ChangeAddrForItinerary Error");
                        ...
                        requestEnd: function (e) {
                            if (e.response) {
                                if (e.response.MsgType === 'error') {
                                    ...
                                    console.log("B3_OsobyDalsiKontakt")
    ...
    function MoveToOSN(items, dat, nochange, name, callback) {
        ...
        var l = function (arr) {
            var item = arr[i];
            $.get('@Url.Action("Run_5xto30", "Api/Service")', { iDspisu: item.IDSpisu, Dat: kendo.toString(dat, "yyyy-MM-dd HH:mm:ss"), nochange: nochange }, function (result) {
                counter++
                value = parseFloat(parseInt(counter, 10) * 100) / parseInt(total, 10);
                progressModel.set("progressValue", value);
                if (result.MsgType === "error") {
                    console.log("Run_5xto30 Error")
        ...
        $.each(items, function (i, item) {
            jQuery.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.MsgType === "error") {
                        console.log("Run_xxtoDateFV Error");
