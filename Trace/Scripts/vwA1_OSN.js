Spisy: new kendo.data.DataSource({
    ...
    Spisy_dataBound: function (e) {
        ...
        allRows.each(function(){
            ...
            if (di.rr_State > 29 && di.rr_State < 40) {
...
Spisy_change: function (e) {
    ...
    if (di) {
        urlID = di.IDSpisu;
        localStorage.setItem('lastid_osn', di.IDSpisu);
...
noChangeDate: function(e) {
    var checked = $(e.currentTarget).prop("checked");
    var model = this;
    SetVisitDatePlanNoChange(e.data.IDSpisu, checked, function(result) {
...
btnDateOSNPlan: function (e) {
    ...
    grid.select(row);
    showOSNDatePlan(e.data.DateOSNPlan, e.data.DateOSNMax, e.data.VisitDatePlanNoChange, function (date, nochange, name) {
        DatePlanProcess([e.data], date, nochange, name, function(result) {
            urlID = e.data.IDSpisu; 
            $.get(apiUrl + "vwA1_OSNPodleIDSpisyOsoby", {IDSpisyOsoby: e.data.IDSpisyOsoby}, function(result){
                $.each(result.Data, function(a, b) {
                    var item = model.Spisy.get(b.IDSpisu);
                    if (item) {
                        item.set("VisitDatePlanNoChange", b.VisitDatePlanNoChange);
                        item.set("AlertExist", b.AlertExist);
                        item.set("DateOSNPlan", b.DateOSNPlan);
                    }
                });
            });
        });
