SpisyDL_detailInit: function (e) {
    ...
    var detailModel = kendo.observable({
        ...
        btnDateOSNPlan: function (e) {
            ...
            grid.select(row);
            showOSNDatePlan(e.data.VisitDatePlan, e.data.DateOSNMax, e.data.VisitDatePlanNoChange, function (date, nochange) {
                DatePlanProcess([e.data], date, nochange, function(result) {
                    urlID = e.data.IDSpisu; 
                    $.get(apiUrl + "vwA2_DLOSNSubPodleIDSpisyOsoby", {IDSpisyOsoby: e.data.IDSpisyOsoby}, function(result){
                        $.each(result.Data, function(a, b) {
                            ...
                            if (item) {
                                ...
                                item.set("DateOSNPlane", b.DateOSNPlane);
                            }
                        });
                    });
                });
