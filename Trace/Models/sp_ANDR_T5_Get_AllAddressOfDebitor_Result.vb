'------------------------------------------------------------------------------
' <auto-generated>
'    Tento kód byl generován ze šablony.
'
'    Ruční změny tohoto souboru mohou způsobit neočekávané chování v aplikaci.
'    Ruční změny tohoto souboru budou přepsány, pokud bude kód vygenerován znovu.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic

Partial Public Class sp_ANDR_T5_Get_AllAddressOfDebitor_Result
    Public Property IDSpisu As Integer
    Public Property IDSpisyOsobyAdresy As Integer
    Public Property AddressTypeTxt As String
    Public Property Jmeno As String
    Public Property Adresa As String
    Public Property rr_AddressType As Nullable(Of Short)
    Public Property rr_AddressValidity As Nullable(Of Short)
    Public Property AddressValidityTxt As String
    Public Property Poradicko As Nullable(Of Short)
    Public Property PersAddressForItinerary As Boolean

End Class
